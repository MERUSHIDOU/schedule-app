...
**重要:** 
メインエージェントがtmux統合でworktreeを作成し、タスクを移譲した場合、メインエージェントは作業を終了します。
変更点をPRに上げてくださいなどのgit操作だけでも渡してください。ワークツリーを作った場合はワークツリー側のエージェントがgit操作をします。
新しいpaneのClaudeセッションがタスクを引き継ぎます。
