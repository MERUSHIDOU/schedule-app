...
### タスク移譲時のメインエージェント動作（重要）

**メインエージェントがtmux統合でworktreeを作成し、新しいClaudeセッションにタスクを移譲できた場合、メインエージェントは作業を終了すること。**

- タスク移譲成功後、メインエージェントは「タスクを新しいセッションに移譲しました」というメッセージを表示
- それ以上の作業は行わず、ユーザーの次の指示を待つ
- 新しいpaneのClaudeセッションがタスクを引き継ぎ、実装を進める
